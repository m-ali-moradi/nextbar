...
public class InternalRequestVerificationFilter extends OncePerRequestFilter {
    ...
    private static final String HMAC_ALGORITHM = "HmacSHA256"; // Use a strong HMAC algorithm
    private static final String INTERNAL_SERVICE_HEADER = "X-Internal-Service"; // Header to identify the calling service, e.g., "gateway"
    private static final String INTERNAL_TIMESTAMP_HEADER = "X-Internal-Timestamp"; // Header to prevent replay attacks, should contain a timestamp
    private static final String INTERNAL_SIGNATURE_HEADER = "X-Internal-Signature"; // Header containing the HMAC signature of the request
    private static final long MAX_CLOCK_SKEW_MS = 5 * 60 * 1000; // 5 minutes allowed clock skew for timestamp validation

    // These values should be configured in application properties for security and flexibility
    @Value("${security.internal.enabled}")
    private boolean enabled;
    
    // The secret key used for signing should be a strong, random value and kept secure
